...
## Kick combat state for the enemy boss unit.
##
## Tracks physical frames during a kick animation to trigger active collision 
## queries and implements a cooldown guard to control combat pacing.


## State to transition to once the kicking animation completes.
...
func _on_frame_changed() -> void:
	...
	if actor.sprite.animation == "kick" and actor.sprite.frame == hit_frame:
...
func _on_animation_finished() -> void:
	...
	if actor.sprite.animation == "kick":
